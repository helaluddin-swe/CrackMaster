      ...
      <div className="relative z-10 w-full">
        ...
        <div className="flex items-center gap-3 sm:gap-4 mb-3 sm:mb-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className={`text-[9px] sm:text-[10px] font-bold uppercase tracking-widest mt-0.5 sm:mt-1 ${theme.metaText}`}>
              {totalQuestions > 0 ? `${totalQuestions.toLocaleString()} টি প্রশ্ন` : totalQuestions === 0 ? '০ টি প্রশ্ন' : 'লোড হচ্ছে...'}
            </p>
          </div>
        </div>

        {/* Sub-Topic Tags - Responsive inline wrapping */}
        {!subject.isSpecial && subject.tags && (
          <div className="flex flex-wrap items-center gap-1 sm:gap-1.5 mb-4 sm:mb-5 max-h-12 overflow-hidden">
            {subject.tags.slice(0, 3).map((tag, i) => (
              <span 
                key={i} 
                className={`px-2 py-0.5 text-[8px] sm:text-[9px] font-bold rounded-md border transition-colors ${theme.tagBg}`}
              >
                #{tag}
              </span>
            ))}
            {subject.tags.length > 3 && (
              <span className={`text-[8px] sm:text-[9px] font-bold pl-0.5 ${theme.tagOverflow}`}>
                +{subject.tags.length - 3}
              </span>
            )}
          </div>
